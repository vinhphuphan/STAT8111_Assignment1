...
<!-- According to the histogram, the distributions of `pemax`, `weight`, `bmp`, `fev1`,`rv`, `frc` are right-skewed. Hence, there is a need of transformation. The appropriate transformation for right-skewed distribution is log transformation. -->

<!-- **After log transformation** -->
```{r histogram log, include=FALSE}
par(mfrow = c(2,3))
hist(log(data$pemax),
    main="Distribution of log(pemax)",
    col="#98c1d9")
hist(log(data$weight),
    ...
    col="#98c1d9")
hist(log(data$fev1),
    main="Distribution of log(fev1)",
    col="#98c1d9")
hist(log(data$bmp),
    main="Distribution of log(bmp)",
    xlab="% of normal",
    col="#98c1d9")
hist(log(data$rv), 
...
hist(log(data$frc), 
     ...
     col="#98c1d9")
```
<!-- Now, the distribution of log(`pemax`) is seem to be better than the distribution of `pemax`. -->
...
\[ f(x ; \mu, \gamma) = \exp\left(\frac{1}{2}log(\frac{\gamma}{2\pi x^3})-\frac{\gamma}{2\mu^2}\frac{(x - \mu)^2}{x}\right) \]

\[ f(x ; \mu, \gamma) = \exp\left(\frac{1}{2}log(\frac{\gamma}{2\pi x^3})-\frac{\gamma}{2\mu^2}\frac{(x^2 - 2x\mu + \mu^2)}{x}\right) \]

\[ f(x ; \mu, \gamma) = \exp\left(\frac{1}{2}log(\frac{\gamma}{2\pi x^3})-\frac{\gamma x}{2\mu^2}+\frac{x}{\mu} +\frac{\gamma}{2x}\right) \]

\[ f(x ; \mu, \gamma) = \exp\left(-\frac{\gamma x}{2\mu^2}+\frac{x}{\mu}+\frac{1}{2}log(\frac{\gamma}{2\pi x^3}) +\frac{\gamma}{2x}\right) \]
...
with: \[\phi = \gamma , \> a = \frac{1}{\phi}, \> \theta = -\frac{1}{2\mu^2}, \>b = -\sqrt{-2\theta} , c = \frac{1}{2}log(\frac{\theta}{2\pi x^3}) +\frac{\theta}{2x}\].
...
For the Inverse Gaussian distribution, the natural parameter \[\theta\] is given by:

\[\theta = -\frac{1}{2\mu^2}\]

**Scale Parameter** \[\phi\]:

In this case, the scale parameter \[\phi\] corresponds to \[\gamma\], which is a parameter of the Inverse Gaussian distribution:

\[\phi = \gamma\]

Therefore, in the exponential family form, the natural parameter is \[\mu\], and the scale parameter is \[\gamma\] of the Inverse Gaussian distribution.
